...
@Test func aRoleIsStoredAgainstTheDeviceItBelongsTo() {
    let doc = DashboardDocument()
        .settingBinding("binary_sensor.gc", role: .closedLimit, for: "cover.garage")
    #expect(doc.tileBindings["cover.garage"]?[.closedLimit] == "binary_sensor.gc")
    #expect(doc.tileBindings["cover.garage"]?[.openLimit] == nil)
}

/// A binding is a fourth decision about the same device and must not erase the other three.
@Test func aBindingSurvivesTheOtherDecisionsAboutADevice() {
    let doc = DashboardDocument()
        .settingDisplayName("Garage", for: "cover.garage")
        .settingSize(TileSpan(columns: 2, rows: 2), for: "cover.garage", on: .overview)
        .settingStateStyle(.label, for: "cover.garage")
        .settingBinding("binary_sensor.gc", role: .closedLimit, for: "cover.garage")
        .settingBinding("binary_sensor.go", role: .openLimit, for: "cover.garage")
    #expect(doc.displayNames["cover.garage"] == "Garage")
    #expect(doc.tileSizes["cover.garage"]?[.overview] == TileSpan(columns: 2, rows: 2))
    #expect(doc.tileStateStyles["cover.garage"] == .label)
    #expect(doc.tileBindings["cover.garage"]?.count == 2)
}

@Test func clearingTheLastBindingRemovesTheRecord() {
    let doc = DashboardDocument()
        .settingBinding("binary_sensor.gc", role: .closedLimit, for: "cover.garage")
        .settingBinding(nil, role: .closedLimit, for: "cover.garage")
    #expect(doc.tileBindings["cover.garage"] == nil)
    #expect(doc.raw.asObject?["entities"] == nil)
}

/// A role a newer build understands is dropped rather than guessed at.
@Test func anUnreadableRoleIsIgnored() {
    let doc = DashboardDocument(raw: .object([
        "schema": .int(DashboardDocument.schema),
        "entities": .object(["cover.garage": .object([
            "bindings": .object(["tilt_limit": .string("binary_sensor.x")])])])]))
    #expect(doc.tileBindings["cover.garage"] == nil)
}
